...
async function transcribeAudioWithLemonfox(videoUrl, lemonfoxApiKey) {
    ...
    try {
        ...
        return null; // On renvoie null en cas d'échec pour ne pas bloquer l'analyse
    }
}
...
function calculateLemonfoxConfidence(transcriptionData) {
    ...
    const confidenceScores = transcriptionData.segments.map(s => s.avg_logprob || s.confidence || 0).filter(s => s !== 0);
    if (confidenceScores.length === 0) return 0.5;
    const avgConfidence = confidenceScores.reduce((sum, score) => sum + score, 0) / confidenceScores.length;
    return avgConfidence < 0 ? Math.max(0, Math.min(1, Math.exp(avgConfidence))) : Math.max(0, Math.min(1, avgConfidence));
}

function assessLemonfoxQuality(transcriptionData) {
    let qualityScore = 100;
    const text = transcriptionData.text || '';
    if (text.length < 10) qualityScore -= 40;
    if ((transcriptionData.segments || []).length === 0) qualityScore -= 30;
    [/\[.*?\]/g, /\(.*?\)/g, /\.{3,}/g, /\s{3,}/g].forEach(p => { qualityScore -= (text.match(p) || []).length * 5; });
    if (transcriptionData.words && transcriptionData.words.length > 0) qualityScore += 10;
...
async function enrichLemonfoxTranscription(transcriptionData) {
    ...
    const hooksDetected = detectHooksWithTimestamps(segments);
    const ctaDetected = detectCTAsWithTimestamps(segments);
    ...
    return {
        ...
        viralscope_insights: generateViralScopeAudioInsights(segments, hooksDetected, ctaDetected),
        audio_optimization: generateAudioOptimizationTips(speechPatterns, temporalAnalysis, hooksDetected, ctaDetected)
    };
}
...
function analyzeTemporalPatterns(segments, words) {
    ...
    return { total_duration: totalDuration, words_per_minute: wpm, speech_rhythm: wpm > 170 ? 'rapide' : wpm > 120 ? 'normal' : 'lent' };
}

function detectHooksWithTimestamps(segments) {
    const hooks = [];
    const patterns = [
        { p: /^(pourquoi|comment|qui|que|quoi|où|quand)/i, t: 'question' },
        { p: /(secret|astuce|méthode|technique)/i, t: 'secret' },
        { p: /(incroyable|choc|fou|dingue)/i, t: 'emotion' },
        { p: /(personne ne|jamais|interdit)/i, t: 'controversial' }
    ];
...
function detectCTAsWithTimestamps(segments) {
    const ctas = [];
    const patterns = [
        { p: /(abonne|follow|s'abonner)/i, t: 'subscribe' },
        { p: /(like|j'aime|double.*tap)/i, t: 'like' },
        { p: /(commente|commentaire|dis.*moi)/i, t: 'comment' },
        { p: /(partage|share|montre)/i, t: 'share' }
    ];
    segments.forEach(seg => {
        patterns.forEach(({ p, t }) => {
...
function getEmptyAnalysis() {
    return { viralscope_insights: ['❌ Échec de la transcription audio'], audio_optimization: ['🔧 Vérifier la qualité audio de la vidéo'] };
}
  ...
  try {
    ...
    **Contenu textuel & visuel :** Description: "${videoData.description}".`;
    ...
    const finalResponse = {
        ...
        transcription: audioAnalysis // On ajoute la transcription complète pour le frontend
    };
