...
export default async function handler(req, res) {
  ...
  try {
    // --- ÉTAPE 1: SCRAPING COMPLET (PLAN A) ---
    ...
    if (SCRAPINGBEE_API_KEY) {
        try {
            ...
            if (!response.ok) throw new Error(`ScrapingBee a échoué (status: ${response.status}), passage au Plan B.`);
            
            const html = await response.text();
            const scriptTagContent = html.split('<script id="SIGI_STATE" type="application/json">')[1]?.split('</script>')[0];
            if (!scriptTagContent) throw new Error("SIGI_STATE non trouvé, passage au Plan B.");
            ...
            console.warn(scrapingError.message);
            videoData = {}; // On réinitialise pour être sûr de passer au Plan B
        }
    }

    // --- PLAN B: RETRAITE TACTIQUE SUR OEMBED (si le Plan A a échoué) ---
    if (!videoData.stats) {
